...
The takeaway is that local additivity in the residual stream does not imply prompt compressibility. Role prompts are two things at once: an additive composition that becomes mechanistically tractable at one small, well-defined region --- a natural site for interpretability and for fine-grained steering of persona or task --- and a distributed prompt/KV mechanism that produces persona-conditioned behavior across the full continuation and that local activation arithmetic cannot displace.
\end{abstract}
...
The natural reading of an additive decomposition is that the role prompt can be replaced by its cached vector --- a single residual injected once should reproduce the long-persona behavior. The same intervention machinery shows that it cannot. Injecting the cached additive prediction at $p_{\text{last}}$ into a baseline host prompt with the persona text removed does not approach the clean long-persona target, and clamping the substitution at many layers, or substituting the oracle clean residual $h_{XY}$ itself, does not change this. Multi-token persona-conditioned generation flows through attention back to the persona-text positions throughout the prompt, which no residual at one site reproduces. Local additivity in the residual stream does not imply prompt compressibility; role prompts operate at both scales simultaneously.
...
Persona-task composition in instruction-tuned language models admits a localized additive description in the residual stream near answer formation, holds across Gemma-2-2B-IT and Qwen-2.5-\{1.5B, 3B\}-Instruct on a 48-cell long-persona grid, and survives a behavioral test of persona-conditioned output content. The natural reading of this -- that the role prompt can be replaced by the cached vector -- is wrong, and the same intervention machinery shows why: persona-conditioned multi-token generation depends on attention back to the persona-text positions, and no residual at one site can reproduce that. Local additivity in the residual stream does not imply prompt compressibility. The natural next step is to characterize the distributed prompt/KV mechanism directly, and to understand the precise interface between it and the local additive regime.
